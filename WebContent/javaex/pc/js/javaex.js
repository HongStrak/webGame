...
//						html += '<a href="javascript:;" class="fold">查看更多回复<i class="icon-keyboard_arrow_right"></i></a>';
			...
			javaexShowReplyTextarea1 : function(obj, args) {
				...
				if (posrLength==0) {
					var html = '';
					html += '<div class="posr">';
					html += '<textarea wrap="virtual" class="comment-textarea comment-textarea-reply"></textarea>';
					html += '<div style="text-align:right;">';
			...
			alert : function(args) {
				...
				// 弹出层代码
				var alertHtml = '<div id='+UUID +' class="dialog animated zoomIn" style="width:'+width+'px;top:'+settings.top+';left:'+(document.documentElement.clientWidth-width)/2+'px;">';
			...
			confirm : function(args) {
				...
				// 弹出层代码
				var confirmHtml = '<div id="'+UUID+'" class="dialog animated zoomIn" style="width:' + width + 'px;top:' + settings.top + ';left:' + (document.documentElement.clientWidth-width)/2 + 'px;">';
			...
			deleteDialog : function(obj, args) {
				...
				// 弹出层代码
				var html = '<div id='+UUID+' class="danger-dialog win animated fadeInUpSmall" style="left:'+parseInt(left-186+(obj.offsetWidth/2)+30)+'px;top:'+parseInt(top+obj.offsetHeight-48)+'px;">';
			...
			optTip : function(args) {
				...
				if (type=="submit") {
					...
					html += '<div class="opt-tip animated fadeInDown" style="top: 40px; left: 50%;">';
					...
					if ($(".tip-icon-loading").length>0) {
						...
						// 修改文字
						$(".opt-tip .tip-msg").text(settings.content);
					} else {
						$(".opt-tip").remove();
						
						html += '<div class="opt-tip animated fadeInDown" style="top: 40px; left: 50%; background-color:#79c37b;">';
						html += '<div style="display: flex;">';
					...
					if ($(".tip-icon-loading").length>0) {
						...
						html += '<div class="opt-tip animated fadeInDown" style="top: 40px; left: 50%; background-color:#ff6e6e;">';
						html += '<div style="display: flex;">';
			...
			loading : function(args) {
				...
				if (type=="") {
					...
					if (containerId=="") {
						// 位于整个页面
						if (id=="") {
							$(document.body).append('<div class="javaex-loading">'+content+'</div>');
						} else {
							if (id=="") {
								$(document.body).append('<div class="javaex-loading">'+content+'</div>');
							} else {
								$(document.body).append('<div id="'+id+'" class="javaex-loading">'+content+'</div>');
							}
						}
					} else {
						// 指定区域
						...
						if (id=="") {
							$("#"+containerId).append('<div class="javaex-loading" style="top:'+top+';">'+content+'</div>');
						} else {
							$("#"+containerId).append('<div id="'+id+'" class="javaex-loading" style="top:'+top+';">'+content+'</div>');
						}
					}
				}
			},
			...
			dialog : function(args) {
				...
				if (type=="image") {
					...
					dialogHtml = '<div id='+UUID+' class="dialog animated zoomIn" style="width:'+width+'px;height:'+height+'px;left:'+(maxWidth-width+120)/2+'px;">';
					...
					dialogHtml += '<div id="box-image-manga" style="width:'+width+'px;height:'+height+'px;left:'+(maxWidth-width+120)/2+'px;top:'+(maxHeight-height+120)/2+'px;">';
					...
					info.addMask();
					dialogHtml = '<div id=' + UUID + ' class="dialog animated zoomIn" style="box-shadow: none;width:' + width + 'px;left:' + (document.documentElement.clientWidth-width)/2 + 'px;">';
			...
			close : function(UUID) {
				if (!UUID) {
					...
					if ($("#"+UUID).hasClass("area-window")) {
						$("#"+UUID).remove();
						$(".allcover").remove();
					} else if ($("#"+UUID).hasClass("danger-dialog")) {
			...
			iframeMenu : function(args) {
				$(".admin-iframe-content ul.menu>li>a").click(function () {
					if (!$(this).parent("li").hasClass("menus")) {
